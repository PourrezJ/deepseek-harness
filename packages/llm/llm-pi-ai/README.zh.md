...
发往 `opencode-go` 的请求携带由 seam 的 `sessionId` 派生的 `x-opencode-session`，因为 OpenCode Go 按会话路由并缓存提示词；主循环、压缩与会话标题调用都会标记该身份，因此一次会话发送一个稳定值。部署配置的 `x-opencode-session` 优先生效，未携带会话 id 的请求不发送该头。
